...
BEGIN	
	...
	-- Get included suite
	SELECT test_suite_id INTO testSuiteIds FROM test_run_has_test_suite WHERE test_run_id = testRunId;
    SET control = 5;
    
-- DROP TABLE IF EXISTS all_test_suite_ids_in_test_run;
-- CREATE TEMPORARY TABLE all_test_suite_ids_in_test_run SELECT 1=0 AS ts_row_id;
...
-- 			WHERE 		id IN(testSuiteIds) 
		...
		ELSE			   
			INSERT INTO all_test_suite_ids_in_test_run SELECT ts_row_id FROM test_suite_has_children;
		END IF;*/
		...
		-- END IF;
        IF control <= 0 THEN			LEAVE loop_suites_and_elements;		END IF;
