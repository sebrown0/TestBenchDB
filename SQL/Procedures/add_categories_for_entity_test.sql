...
BEGIN
	...
	IF catId > 1 THEN
		CALL add_category_for_entity_test(catId, 1, entityTestId, entityTestEntityTestId);
	END IF; 
     
	SET catId = get_test_cat_id_for_name(c2);
    IF catId > 1 THEN  
		CALL add_category_for_entity_test(catId, 2, entityTestId, entityTestEntityTestId);
	END IF;
END